...
static moveit::core::RobotModelPtr getRobotModel(std::string robot_description)
{
  static std::mutex mutex;
  std::lock_guard<std::mutex> lock(mutex);

  if(robot_description.empty()) robot_description = "robot_description";

  static std::unordered_map<std::string, moveit::core::RobotModelPtr> robot_models;
  ...
  if(robot_models.find(robot_description) == robot_models.end())
  {
    ...
    //std::string solver = "bio_ik_kinematics_plugin/BioIKKinematicsPlugin";
    //robot_model_loader.loadKinematicsSolvers(kinematics_plugin_loader::KinematicsPluginLoaderPtr(new kinematics_plugin_loader::KinematicsPluginLoader(solver, 0.005, 1)));
    static std::unordered_map<std::string, robot_model_loader::RobotModelLoader> loaders;
...
static bool getPositionIK(moveit_msgs::GetPositionIK::Request& request, moveit_msgs::GetPositionIK::Response& response, bool approximate)
{
  ...
  if(!request.ik_request.robot_state.joint_state.name.empty() || !request.ik_request.robot_state.multi_dof_joint_state.joint_names.empty())
  {
    moveit::core::robotStateMsgToRobotState(request.ik_request.robot_state, robot_state);
    robot_state.update();
  }
  ...
  if(request.ik_request.pose_stamped_vector.empty())
  {
    ...
    {
      success = robot_state.setFromIK(joint_model_group, request.ik_request.pose_stamped.pose, request.ik_request.attempts, request.ik_request.timeout.toSec(), moveit::core::GroupStateValidityCallbackFn(), ik_options);
    } else {
      success = robot_state.setFromIK(joint_model_group, request.ik_request.pose_stamped.pose, request.ik_request.ik_link_name, request.ik_request.attempts, request.ik_request.timeout.toSec(), moveit::core::GroupStateValidityCallbackFn(), ik_options);
    }
  } else {
    ...
    if(request.ik_request.ik_link_names.empty())
    {
      ...
      success = robot_state.setFromIK(joint_model_group, poses, end_effector_names, request.ik_request.attempts, request.ik_request.timeout.toSec(), moveit::core::GroupStateValidityCallbackFn(), ik_options);
    } else {
      success = robot_state.setFromIK(joint_model_group, poses, request.ik_request.ik_link_names, request.ik_request.attempts, request.ik_request.timeout.toSec(), moveit::core::GroupStateValidityCallbackFn(), ik_options);
    }
  }
...
static bool getPositionIK(moveit_msgs::GetPositionIK::Request& request, moveit_msgs::GetPositionIK::Response& response)
{
  return getPositionIK(request, response, false);
}

static bool getApproximateIK(moveit_msgs::GetPositionIK::Request& request, moveit_msgs::GetPositionIK::Response& response)
{
...
static void convertGoals(const bio_ik_msgs::IKRequest& ik_request, bio_ik::BioIKKinematicsQueryOptions& ik_options)
{
  ...
  {
    ik_options.goals.emplace_back(new bio_ik::ConeGoal(m.link_name, p(m.position), w(m.position_weight), p(m.axis), p(m.direction), m.angle, w(m.weight)));

  }
}

static bool getBioIK(bio_ik_msgs::GetIK::Request& request, bio_ik_msgs::GetIK::Response& response)
{
  auto robot_model = getRobotModel("");
  if(!robot_model)
  {
  ...
  bool success = robot_state.setFromIK(joint_model_group, EigenSTL::vector_Affine3d(), std::vector<std::string>(), request.ik_request.attempts, request.ik_request.timeout.toSec(), moveit::core::GroupStateValidityCallbackFn(), ik_options);

  robot_state.update();

  moveit::core::robotStateToRobotStateMsg(robot_state, response.ik_response.solution);
...
int main(int argc, char** argv)
{
  ...
  getRobotModel("robot_description");
